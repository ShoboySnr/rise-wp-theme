...
class USIN_Query{
		...
		if(!empty($this->filters)){
			foreach ($this->filters as $filter) {
				if(isset($filter->by) && isset($this->db_map[$filter->by]) && 
					(isset($filter->condition) || in_array($filter->operator, array('isnull', 'notnull', 'isset', 'notset', 'isset_ser', 'notset_ser')))){
					if($filter->by=='role'){
	...
	protected function add_text_match_filter($filter, &$clause){
		if($this->is_condition_set($filter)){
			global $wpdb;
			$db_ref = $this->db_map[$filter->by]['db_ref'];
			$operator = $filter->operator == 'is' ? '=' : '!=';

			$clause .= $wpdb->prepare(" AND $db_ref $operator %s", $filter->condition);
		}
	}
	...
	protected function add_serialized_search_filter($filter, &$clause){
		if($this->is_condition_set($filter)){
			...
			if($filter->operator == 'contains_ser'){
				...
				$clause .= $wpdb->prepare(" AND ($db_ref LIKE %s OR $db_ref LIKE %s)", $serialized_condition, $condition);
			}else{
				$empty_values = USIN_Helper::array_to_sql_string($this->empty_serialized_values);
				$clause .= $wpdb->prepare(" AND (($db_ref NOT LIKE %s AND $db_ref NOT LIKE %s) OR $db_ref IS NULL)", $serialized_condition, $condition);
			}
	...
	protected function add_comma_search_filter($filter, &$clause){
		...
		if($filter->operator == 'contains_com'){
			$clause .= $wpdb->prepare(" AND ($db_ref REGEXP %s)", $condition);
		}else{
			$clause .= $wpdb->prepare(" AND ($db_ref NOT REGEXP %s OR $db_ref IS NULL)", $condition);
		}
	}
	...
	protected function add_null_filter_ser($filter, &$clause){
		if(isset($this->db_map[$filter->by])){
			...
			if($filter->operator == 'isset_ser'){
				$clause .= " AND $db_ref IS NOT NULL AND $db_ref NOT IN ($empty_values)";
			}elseif ($filter->operator == 'notset_ser'){
				$clause .= " AND ($db_ref IS NULL OR $db_ref IN ($empty_values))";
			}	
		}
	}
	...
	protected function add_date_number_filter($filter, &$clause){
		...
		if($filter->type == 'date'){
			...
			if(isset($map['null_to_zero']) && $map['null_to_zero']===true){
				$ref = 'IFNULL('.$ref.', 0)';
			}elseif(isset($map['stores_empty_str']) && $map['stores_empty_str'] === true){
				$clause .= " AND $ref != ''";
			}
