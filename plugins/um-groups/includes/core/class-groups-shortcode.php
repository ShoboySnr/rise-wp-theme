...
class Groups_Shortcode {
	...
	public function discussion_activity( $atts = array() ) {
		...
		if ( ! is_user_logged_in() || ( $privacy != 'public' &&  UM()->Groups()->api()->has_joined_group( get_current_user_id() , $um_group_id ) != 'approved' ) ) {
			return '<div class="um-groups-widget">' . __("You do not have the necessary permission for the specified Group to perform this action.", 'um-groups' ) . '</div>';
		}
		...
		$is_single_post = (boolean) $args['wall_post'];

		$can_write = UM()->Groups()->discussion()->can_write() && ! $is_single_post &&
		             UM()->Groups()->api()->has_joined_group( $args['user_id'], $args['group_id'] ) == 'approved';
	...
	public function invite_list( $atts ) {
		...
		if ( $invites_enabled && ! UM()->Groups()->api()->can_invite_members( $um_group_id, null ) ) {
			return '<div class="um">' . __("You can't invite users to this group, you are not approved member.", 'um-groups' ) . '</div>';
