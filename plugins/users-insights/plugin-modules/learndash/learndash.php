		...
		return array(
			...
			'settings' => array(
				'enable_course_analytics' => array(
					'name' => __('Enable per course analytics for courses', 'usin'),
					'type' => USIN_Settings_Field::TYPE_CHECKBOXES,
					'options' => $this->is_modules_page() ? self::get_items(self::COURSE_POST_TYPE, true) : array(),
					'desc' => __('For each selected course the following additional features will be available: course started and course completed date
					columns in the user table, as well as Number of students started/completed course over time reports in the Reports section.', 'usin')
				),
