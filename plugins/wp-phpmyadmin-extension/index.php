...
namespace WpPhpMyAdminExtension
{
  ...
  class PluginClass extends \Puvox\wp_plugin
  {
        ...
        [
            ...
            'menu_pages'             => [
                'first' =>[
                    ...
                    'level'           => 'mainmenu', 
                    'icon'            => $this->helpers->baseURL.'/assets/media/menu_icon.png" style="width:30px;', 
    ...
    public function setup_files()
    {
        ...
        try{
            ...
            else
            {
                ...
                $this->file_put_contents($a, '<?php //'.$suffix);
            }
        }
        catch(\Exception $ex){
            _e("DIRECTORY SEEMS NOT WRITABLE! WP-PHPMYADMIN PLUGIN WILL NOT WORK");
            echo $ex->getMessage();
            ...
            { 
                if( is_dir($this->pma_abspath_real) && !is_dir($this->pma_abspath) ) //check again after LOCK 
                {
                    ...
                    if ( stripos($cont,$flag) === false )
                    {
                        ...
                        {
							wp_die("internal issue if Wp phpMyAdmin plugin, as seems PMA changed their internal approach");
                        }
    ...
    { 
        if( isset($_GET['goto_wp_phpmyadmin']) ){ 
            ...
            { 
                ...
                wp_die("You do not have enough privilegges to open this page.");
            }
        }
        return;
    }
    ...
    {  
        ...
        else _e("WP-PHPMYADMIN can not work on your server");
        ?>
        ...
        { 
            ...
            jQuery(function(){ 
                ...
                jQuery("#old_pma_install").on("click", function(e){
                    ...
                    window.setTimeout( function(){ PuvoxLibrary.message("<?php _e('Please don\'t refresh the page untill it refreshes automatically (might need up to 45 seconds).', 'wp-phpmyadmin-extension');?>"); }, 2000);
    ...
    {
        $dir = dirname($path);
        if(!is_dir($dir))  $this->helpers->mkdir_recursive($dir);
        ...
        if ( is_writable( $dir ) ){
            ...
            throw new \Exception("This plugin don't have permission to write file at: $path");
        }
    }
